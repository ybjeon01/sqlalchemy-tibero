...
class VARCHAR2(sqltypes.VARCHAR):
...
class FLOAT(sqltypes.FLOAT):
    ...
    a Tibero-specific :paramref:`_tibero.FLOAT.binary_precision`
    parameter is accepted, and
    the :paramref:`_sqltypes.Float.precision` parameter is not accepted.

    Tibero FLOAT types indicate precision in terms of "binary precision", which
    ...
    def __init__(
        ...
        :param binary_precision: Tibero binary precision value to be rendered
         ...
         The default value used by Tibero for FLOAT / DOUBLE PRECISION is 126.
...
class _TiberoDateLiteralRender:
...
class DATE(_TiberoDateLiteralRender, sqltypes.DateTime):
...
class _TiberoDate(_TiberoDateLiteralRender, sqltypes.Date):
    def literal_processor(self, dialect):
...
class TIMESTAMP(sqltypes.TIMESTAMP):
    """Tibero implementation of ``TIMESTAMP``, which supports additional
    Tibero-specific modes
    ...
    def __init__(self, timezone: bool = False, local_timezone: bool = False):
        """Construct a new :class:`_tibero.TIMESTAMP`.

        :param timezone: boolean.  Indicates that the TIMESTAMP type should
         use Tibero's ``TIMESTAMP WITH TIME ZONE`` datatype.

        :param local_timezone: boolean.  Indicates that the TIMESTAMP type
         should use Tibero's ``TIMESTAMP WITH LOCAL TIME ZONE`` datatype.
