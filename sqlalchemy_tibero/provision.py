...
def _tib_drop_ignore(conn, dbname):
    try:
...
def _tibero_drop_db(cfg, eng, ident):
    with eng.begin() as conn:
        _tib_drop_ignore(conn, ident)
        _tib_drop_ignore(conn, "%s_ts1" % ident)
        _tib_drop_ignore(conn, "%s_ts2" % ident)


@stop_test_class_outside_fixtures.for_db("tibero")
...
@post_configure_engine.for_db("tibero")
def _tibero_post_configure_engine(url, engine, follower_ident):
...
def _reap_tibero_dbs(url, idents):
    ...
    with eng.begin() as conn:
        ...
        for total, username in enumerate(to_drop, 1):
            if _tib_drop_ignore(conn, username):
                dropped += 1
        log.info(
